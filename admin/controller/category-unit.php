...
$description='Web site phát triển bởi anh Hợp đẹp trai';
	if($form=='')
	{	
		$link='index.php?page=category-unit';
		//Doi trang thai
		if($id>0 && isset($_REQUEST['status']))
		{
		   $tt=$_REQUEST['status'];
		   if($tt==0) $newtt=1; else $newtt=0;
		   mysql_query('UPDATE tb_unit_category set publish='.$newtt.' where id='.$id);
		}
		...
		if($id>0 && isset($_REQUEST['in_home']))
		{
		   $tt=$_REQUEST['in_home'];
		   if($tt==0) $newtt=1; else $newtt=0;
		   mysql_query('UPDATE tb_unit_category set in_home='.$newtt.' where id='.$id);
		...
		if($id>0 && isset($_REQUEST['in_sidebar']))
		{
		   ...
		   if($tt==0) $newtt=1; else $newtt=0; 
		   mysql_query('UPDATE tb_unit_category set in_sidebar='.$newtt.' where id='.$id);
		...
		if(($id>0 && $action=='del'))
			{
				...
				$str_count="SELECT * FROM tb_unit_category WHERE  parentid=".$id;$_count=$lib->selectall($str_count);
				if($lib->total($_count)>0) $lib->thongbao('Vẫn còn đề thi con  !','danger' );
				else
				{
					$str_count1="SELECT * FROM tb_unit_item WHERE  parentid=".$id;$_count1=$lib->selectall($str_count1);
					if($lib->total($_count1)>0) $lib->thongbao('Vẫn còn bài viết trong đề thi này  !','danger' );
					else
					{
						if($lib->delete('tb_unit_category','id',$id))
						{
		...
		$str_list="SELECT * FROM tb_unit_category where publish >=0 ";
		...
		$str_category="SELECT * FROM tb_unit_category";
		...
		if(isset($_POST['btnGui'])){
			...
			{ 
				...
				if($data['title']!='')
				{
					if(is_numeric($lib->insert('tb_unit_category',$data)))
					{
						$data=null;
			...
			if($id>0 && $action=='edit') 
			{
				$redirect=!empty($redirect)?base64_decode($redirect):$url.'index.php?page=category-unit';
				$data['alias']=$lib->maketitle($data['title']);
				$data['updated']=gmdate('Y-m-d H:i:s', time() +7*3600 );
				$data['userid_updated']=$loginadmin->id;
				$lib->update('tb_unit_category',$data,$id);
				if($data['title']!='')
				{
					if($lib->update('tb_unit_category',$data,$id))
					{
		...
		if($id!='' && $id>0){
			...
			$detail=$lib->selectone("SELECT * FROM tb_unit_category WHERE id='$id'");
			if($detail->title=='' && $action!='del')
			{
				$lib->js_header($url.'index.php?page=category-unit&error=1');
			}
		}
