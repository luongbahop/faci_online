...
$description='Web site phát triển bởi anh Hợp đẹp trai';
	if($form=='')
	{	
		$link='index.php?page=category-article';
		//Doi trang thai
		if($id>0 && isset($_REQUEST['status']))
		{
		   $tt=$_REQUEST['status'];
		   if($tt==0) $newtt=1; else $newtt=0;
		   mysql_query('UPDATE tb_article_category set publish='.$newtt.' where id='.$id);
		}
		...
		if($id>0 && isset($_REQUEST['in_home']))
		{
		   $tt=$_REQUEST['in_home'];
		   if($tt==0) $newtt=1; else $newtt=0;
		   mysql_query('UPDATE tb_article_category set in_home='.$newtt.' where id='.$id);
		...
		if($id>0 && isset($_REQUEST['in_sidebar']))
		{
		   ...
		   if($tt==0) $newtt=1; else $newtt=0; 
		   mysql_query('UPDATE tb_article_category set in_sidebar='.$newtt.' where id='.$id);
		...
		if(($id>0 && $action=='del'))
			{
				...
				$str_count="SELECT * FROM tb_article_category WHERE  parentid=".$id;$_count=$lib->selectall($str_count);
				if($lib->total($_count)>0) $lib->thongbao('Vẫn còn danh mục bài viết con  !','danger' );
				else
				{
					$str_count1="SELECT * FROM tb_article_item WHERE  parentid=".$id;$_count1=$lib->selectall($str_count1);
					if($lib->total($_count1)>0) $lib->thongbao('Vẫn còn bài viết trong danh mục bài viết này  !','danger' );
					else
					{
						if($lib->delete('tb_article_category','id',$id))
						{
							$lib->thongbao('Xoá danh mục bài viết thành công !' );
						}else 
						{
							$lib->thongbao('Xoá danh mục bài viết thất bại !','danger' );
						}
					}
				}
		...
		$str_list="SELECT * FROM tb_article_category where publish >=0 ";
		...
		if(isset($_POST['btnGui'])){
			...
			{ 
				...
				if($data['title']!='')
				{
					if(is_numeric($lib->insert('tb_article_category',$data)))
					{
						$data=null;
						$lib->thongbao('Thêm mới danh mục bài viết thành công !' );
					}else
					{
						$lib->thongbao('Thêm mới danh mục bài viết thất bại !','danger' );
					}
				}else
				{
			...
			if($id>0 && $action=='edit') 
			{
				$redirect=!empty($redirect)?base64_decode($redirect):$url.'index.php?page=category-article';
				$data['alias']=$lib->maketitle($data['title']);
				$data['updated']=gmdate('Y-m-d H:i:s', time() +7*3600 );
				$data['userid_updated']=$loginadmin->id;
				$lib->update('tb_article_category',$data,$id);
				if($data['title']!='')
				{
					if($lib->update('tb_article_category',$data,$id))
					{
						$lib->thongbao('Sửa danh mục bài viết thành công !' );
					}else 
					{
						$lib->thongbao('Sửa danh mục bài viết thất bại !','danger' );
					}
				}else
				{
					$lib->thongbao('Bạn chưa nhập tiêu đề !','danger' );
		...
		if($id!='' && $id>0){
			...
			$detail=$lib->selectone("SELECT * FROM tb_article_category WHERE id='$id'");
			if($detail->title=='' && $action!='del')
			{
				$lib->js_header($url.'index.php?page=category-article&error=1');
			}
		}
